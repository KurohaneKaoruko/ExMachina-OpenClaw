...
class CliTests(unittest.TestCase):
    def test_export_settings_writes_settings_bundle(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue((output / "install" / "INTAKE.md").exists())
            self.assertTrue((output / "install" / "intake.template.json").exists())
    ...
    def test_build_command_exports_lite_pack_by_default(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            intake_doc = (output / "openclaw-pack" / "install" / "INTAKE.md").read_text(encoding="utf-8")
            ...
            self.assertIn("openclaw_install_intake", manifest)
            ...
            self.assertTrue((output / "openclaw-pack" / "install" / "INTAKE.md").exists())
            self.assertTrue((output / "openclaw-pack" / "install" / "intake.template.json").exists())
