...
| 连结体 | 用途 | 常见成员 |
| --- | --- | --- |
| `策划连结体` | 需求澄清、任务拆解、路线规划 | 侦察体、拆解体、约束体、路线体 |
...
| `集成连结体` | 接入路径、配置收束、应用入口、文档与交付路径 | 接驳体、配置体、发布体、文档体 |
...
- 焦点：仓库接入、配置收束、应用路径与 OpenClaw 自举应用。
- 连结指挥体：`集成连结指挥体`
- 子个体：
  - `接驳体`：打通仓库链接、目录入口和外部接入方式。
  - `配置体`：整理配置、元数据和运行参数。
  - `发布体`：定义构建、交付、发布或共享方式。
  - `文档体`：把上手方式、结构与示例整理成面向使用者的文档。
